...
class SkFeed(IntegralFeed):
    ...
    Arguments:
        ...
        off_sites: Off-site integrals; dictionary keyed by tuples of the form
            (z₁, z₂, s₁, s₂), where zᵢ & sᵢ are the atomic & shell numbers of
            the interactions, and valued by interpolation entities. such as
            `CubicSpline` instances. Note that z₁ must be less than or equal
            to z₂, see the notes section for further information.
        device: Device on which the feed object and its contents resides.
        dtype: dtype used by feed object.
    ...
    def __init__(self, on_sites: Dict[int, Union[Parameter, Tensor]],
        ...
        # setter rather than within the `__init__` method so that all the
        # necessary safety checks are performed. ────────────┐
        self._on_sites: Dict[int, Parameter] = on_sites    # │
        self.on_sites = on_sites  # <────────────────────────┘
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        Arguments:
            recurse: If True, then yield parameters of this module and all
                submodules. Otherwise, yield only parameters that are direct
        ...
        # This method must be manually implemented as Pytorch can't index
        # modules stored inside regular dictionaries.
    ...
    def on_sites(self, value: Dict[int, Union[Parameter, Tensor]]):
        ...
        # Getters and setters are used for the `on_sites` attribute so that
        # safety checks can be performed prior to permitting an assignment.
    ...
    def _off_site_blocks(self, atomic_idx_1: Tensor, atomic_idx_2: Tensor,
        ...
        for i, l_1 in enumerate(shells_1):
            ...
            for j, l_2 in enumerate(shells_2[o:], start=o):
                # Retrieve/interpolate the integral spline, remove any NaNs
                # due to extrapolation then convert to a torch tensor.
                inte = self.off_sites[(z_1, z_2, i, j)].forward(dist)
