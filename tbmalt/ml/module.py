
from abc import ABC, abstractmethod
from typing import Optional, Dict, Any, Tuple
from functools import wraps
import torch

from tbmalt import Geometry, Basis, Periodic
from tbmalt.physics.dftb.coulomb import Coulomb


def require_args(*r_args, **r_kwargs):

    def registrar(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            return func(*args, **kwargs)

        wrapper.r_args = r_args
        wrapper.r_kwargs = r_kwargs

        return wrapper

    return registrar


def requires_args(func) -> bool:
    """True if function has required arguments."""
    return hasattr(func, 'r_args') & hasattr(func, 'r_kwargs')


def gather_args_from(func: object, src: object) -> (Tuple[Any], Dict[str, Any]):
    """Gather required argument for `func` from `src`.

    Gathers the arguments & keyword arguments required by the function `func`,
    as specified by its `r_args` & `r_kwargs` attributes from the provided
    class instance `src`. All required arguments are assumed to be attributes
    or properties of the `src` object.

    Args:
        func: the function for which arguments and keyword arguments are to be
            gathered.
        src: a class entity from which the required arguments and keyword
            arguments are to be gathered.

    Returns:
        args: a tuple containing the required arguments.
        kwargs: a dictionary containing the required keyword arguments.
    """

    # Ensure this is only called on functions which have required arguments.
    assert requires_args(func)

    # Collect the specified arguments from the source object `src`. Exceptions
    # are manually thrown here to help users identify associated issues.
    args = []
    for arg_name in func.r_args:
        try:
            args.append(getattr(src, arg_name))
        except AttributeError as e:
            raise AttributeError(
                f'Failed to gather the required argument "{arg_name}" '
                f'from the object "{src}"')

    # Repeat the process for the required keyword arguments
    kwargs = {}
    for kwarg_name, kwarg_value in func.r_kwargs.items():
        try:
            kwargs[kwarg_name] = getattr(src, kwarg_value)
        except AttributeError as e:
            raise AttributeError(
                f'Failed to gather the required keyword argument '
                f'"{kwarg_value}" from the object "{src}"')

    return args, kwargs


def call_with_required_args(func, src):
    args, kwargs = gather_args_from(func, src)
    return func(*args, **kwargs)


class Calculator(ABC):

    def __init__(self, dtype: torch.dtype, device: torch.device,
                 mass: Optional[Dict[int, float]] = None):

        self.__dtype = dtype
        self.__device = device

        self._geometry = None
        self._basis = None
        self._periodic = None
        self._coulomb = None

    @property
    def device(self) -> torch.device:
        """The device on which the feed object resides."""
        return self.__device

    @property
    def dtype(self) -> torch.dtype:
        """Floating point dtype used by feed object."""
        return self.__dtype

    @abstractmethod
    def forward(self, cache: Optional[Dict[str, Any]] = None):
        pass

    def __call__(self, geometry: Geometry, basis: Basis,
                 periodic: Optional[Periodic] = None,
                 coulomb: Optional[Coulomb] = None,
                 cache: Optional[Dict[str, Any]] = None):
        """Run the calculator instance.

        Arguments:
            geometry: System(s) upon which the calculation is to be run.
            basis: Orbital information associated with said system(s).
            periodic: Distance matrix and position vectors including periodic
                images.
            coulomb: 1/R matrix for the periodic geometry.
            cache: A cache entity that may be used to bootstrap the calculation.

        Returns:
            total_energy: total energy of the target system(s). If repulsive
                interactions are not considered, i.e. the repulsion feed is
                omitted, then this will be the band structure energy. If finite
                temperature is active then this will technically be the Mermin
                energy.

        """
        # If a new geometry & basis object have been provided then it's assumed
        # that the user wants to use the calculator on a new system. Otherwise
        # assume the user wishes to rerun the calculator on the current system.
        # The latter is useful if rerunning a calculation after updating one or
        # more of the feed objects.

        g_spec, b_spec = geometry is not None, basis is not None

        # Either geometry and basis are passed or neither are. It is not logical
        # to provide only one.
        if all([g_spec, b_spec]) != any([g_spec, b_spec]):
            raise ValueError(
                '"geometry" & "basis" are mutually inclusive arguments; either '
                'both must be provided or neither.')

        # If no geometry/basis objects were provide and none are currently set
        # then no calculation can be run.
        if not g_spec and self.geometry is None:
            raise AttributeError(
                '"geometry" & "basis" objects must be specified before a '
                'calculation can run')

        # Reset the calculator, update the `geometry` & `basis` attributes, if
        # required, then call the `forward` method.
        # reinitialise, and then call the `forward` method.
        if not g_spec:
            self.reset()
        else:
            self.reset()
            self._geometry, self._basis, self._periodic, self._coulomb =\
                geometry, basis, periodic, coulomb

        return self.forward(cache=cache)

    @property
    def is_batch(self):
        """True of operating on a batch of systems."""
        return self.geometry.positions.dim() == 3

    @property
    def is_periodic(self):
        """If there is any periodic boundary conditions."""
        return self.geometry.is_periodic

    @property
    def geometry(self):
        return self._geometry

    @property
    def basis(self):
        return self._basis

    @property
    def periodic(self):
        return self._periodic

    @property
    def coulomb(self):
        return self._coulomb

    @abstractmethod
    def reset(self):
        pass

    def cache(self) -> Dict[str, Any]:
        """
        This returns a cache that may be fed into the next call to bootstrap the
        startup. This is only meaningful when restarting a calculation on a
        system after the calculator has been reset.
        """
        pass